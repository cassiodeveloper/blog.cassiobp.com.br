...
if($options['TabPosition']=='Top' && in_array($options['TabDesign'],array(7,8)))
{
	$fblbHead_top_margin=($options['Height']/2)*-1;
}
if($options['TabPosition']=='Top' && in_array($options['TabDesign'],array(9)))
{
	$fblbHead_top_margin=($options['Height']/2)*-1;
}
...
if($options['TabPosition']=='Bottom' && in_array($options['TabDesign'],array(7,8)))
{
	$fblbHead_top_margin=($options['Height']/2)-90;
}
if($options['TabPosition']=='Bottom' && in_array($options['TabDesign'],array(9)))
{
	$fblbHead_top_margin=($options['Height']/2)-36;
}

?>
<div class="fblbCenterOuter fblbCenterOuterFb <?=($options['VPosition']=='Fixed' ? 'fblbFixed': '')?> fblb<?=$options['Position']?>" style="<?=($options['VPosition']=='Fixed' ? 'margin-top: '.($options['VPositionPx'] ? $options['VPositionPx'] : '0').'px; ': '')?> <?=($options['Position']=='Left' ? 'left: -'.($options['Width']+$options['Border']).'px;' : 'right: -'.($options['Width']+$options['Border']).'px;')?><?=($options['ZIndex'] ? 'z-index: '.$options['ZIndex'].';': '')?>">
	<div class="fblbCenterInner">
		<div class="fblbWrap fblbTheme0 fblbTab<?=$options['TabDesign']?>">
			<div class="fblbForm" style="background: <?=$options['BorderColor']?>; height: <?=$options['Height']?>px; width: <?=$options['Width']?>px; padding: <?=($options['Position']=='Left' ? $options['Border'].'px '.$options['Border'].'px '.$options['Border'].'px 0' : $options['Border'].'px 0 '.$options['Border'].'px '.$options['Border'].'px')?>";">
				<h2 class="fblbHead" style="margin-top: <?=$fblbHead_top_margin?>px; <?=($options['Position']=='Left' ? 'left: '.($options['Width']+$options['Border']).'px;' : 'right: '.($options['Width']+$options['Border']).'px;')?>">Facebook</h2>
